  ...
  $.fn.extend({
    tableExport(options) {
      ...
      if (defaults.type == 'csv' || defaults.type == 'tsv' || defaults.type == 'txt') {
        ...
        function csvString(cell, rowIndex, colIndex) {
          ...
          if (cell !== null) {
            ...
            if (defaults.type == 'tsv') {
              ...
              if (dataString instanceof Date) {result = defaults.csvEnclosure + dataString.toLocaleString() + defaults.csvEnclosure;}
              else {
        ...
        if (defaults.tfootSelector.length) {$rows.push.apply($rows, $(el).find('tfoot').find(defaults.tfootSelector));}
        $($rows).each(function () {
          trData = '';
        ...
        $(el).filter(function () {
          ...
          $hrows.each(function () {
            ...
            if (trData.length > 0) {docData += '<Row>' + trData + '</Row>';}
            rowIndex++
          })
        ...
        $(el).filter(function () {
          ...
          if (defaults.tfootSelector.length) {$rows.push.apply($rows, $table.find('tfoot').find(defaults.tfootSelector));}
        ...
        if (defaults.pdfmake.enabled === true) {
          ...
          if (defaults.tfootSelector.length) {$rows.push.apply($rows, $(this).find('tfoot').find(defaults.tfootSelector));}
      ...
      */
      ...
      function GetColumnNames(table) {
        var result = []
        $(table).find('thead').first().find('th').each((index, el) => {
        ...
        if (defaults.ignoreColumn.length > 0) {
          if ($.inArray(colIndex, defaults.ignoreColumn) != -1 ||
            ...
            colNames.length > colIndex && typeof colNames[colIndex] != 'undefined' &&
              $.inArray(colNames[colIndex], defaults.ignoreColumn) != -1) {result = true;}
        }
        return result
      }
...
Rowspan = 1
                ...
                if ($(this).is('[rowspan]')) {Rowspan = parseInt($(this).attr('rowspan')) || 1;}
                ...
                if (isColumnIgnored(rowLength, colIndex + rowColspan) === false) {
                  ...
                  for (c = 1; c < Colspan; c++) {cellcallback(null, rowIndex, colIndex + c);}
                }
      ...
      function collectImages(cell, elements, teOptions) {
        if (typeof teOptions.images !== 'undefined') {
          elements.each(function () {
            ...
            if (typeof kids !== 'undefined' && kids.length > 0) {collectImages(cell, kids, teOptions);}
          })
        }
      }
      ...
      function loadImages(teOptions, callback) {
        ...
        if (typeof teOptions.images !== 'undefined') {
          for (i in teOptions.images) {if (teOptions.images.hasOwnProperty(i))
      ...
      function drawAutotableElements(cell, elements, teOptions) {
        elements.each(function () {
          ...
          if (typeof kids !== 'undefined' && kids.length > 0) {drawAutotableElements(cell, kids, teOptions);}
        })
      }
      ...
      function parseString(cell, rowIndex, colIndex) {
        ...
        if (cell !== null) {
          ...
          if ($cell[0].hasAttribute('data-tableexport-value')) {
            ...
            htmlData = htmlData ? String(htmlData) : ''
          } else {
            htmlData = $cell.html()
            ...
            if (typeof defaults.onCellHtmlData === 'function') {htmlData = defaults.onCellHtmlData($cell, rowIndex, colIndex, htmlData);}
              ...
              htmlData = ''
              $.each(html, function () {
                if ($(this).is('input')) {htmlData += $cell.find('input').eq(inputidx++).val();}
                else if ($(this).is('select')) {htmlData += $cell.find('select option:selected').eq(selectidx++).text();}
      ...
      function downloadFile(filename, header, data) {
        ...
        if (filename !== false && (ua.indexOf('MSIE ') > 0 || Boolean(ua.match(/Trident.*rv\:11\./)))) {
          ...
          if (DownloadLink) {
            ...
            } else if (header.toLowerCase().indexOf('base64,') >= 0) {DownloadLink.href = header + base64encode(data);}
